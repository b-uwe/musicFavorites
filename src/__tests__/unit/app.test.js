...
describe( 'Express App - Route Handler Unit Tests', () => {
  ...
  describe( 'HTTP request/response logging middleware', () => {
     ...
     */
    test( 'logs successful HTTP requests at info level', async () => {
    ...
    test( 'includes request duration in log', async () => {
      ...
      const logCall = infoSpy.mock.calls.find( ( call ) => call[ 1 ] === 'HTTP request' );
    ...
    /**
     * Test that middleware always uses info level
     */
    test( 'logs with info level in all environments', async () => {
      const infoSpy = jest.spyOn( mf.logger, 'info' ).mockImplementation( () => {
        // Mock implementation to prevent actual logging
      } );
